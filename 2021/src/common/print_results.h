                    ...
                    , const std::string &enabledPart2 = "1|1" )
{
  ...
  for ( std::size_t _part = 0; _part < 2; ++_part )
  {
    ...
    if ( enables[ 1 ][ 0 ] == '1' )
    {
      std::string fullTaskPath = INPUT_FILES "/" + day + "/task_1.txt";

      _part == 0 ? print_task_result( TestedClass().task_1( fullTaskPath ) )
                 : print_task_result( TestedClass().task_2( fullTaskPath ) );
    }
  }
}
